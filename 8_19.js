...
class BinarySearchTree {
    ...
    insertNode(data, inputNode = this.root) {
        if (this.isEmpty()) {
            this.root = new Node(data)
            return this.root
        }
        // if data is greater than current Node,check it's right child
            // if it's right child is null insert the new Node there ,than return the newly inserted Node
        if (data >= inputNode.data) {
            if (inputNode.right == null) {
                inputNode.right = new Node(data);
                return inputNode.right;
            }
            return this.insertNode(data, inputNode.right);
        } else {
            if (inputNode.left == null) {
                inputNode.left = new Node(data);
                return inputNode.left;
            }
            return this.insertNode(data, inputNode.left);
        }
    }
    insert(data, inputNode = this.root) {
        if (inputNode == null){
            this.root = new Node(data);
        }
...
console.log(twoLevelTree.insertNode(17));
console.log(twoLevelTree.insert(4));
