...
class BinarySearchTree {
    ...
    toArrPreorder(arr = [], CurrentNode = this.root) {
        //   get all left side first
        if (CurrentNode != null) {
            arr.push(CurrentNode.data);
            //  left = 15
            return this.toArrPreorder(arr, CurrentNode.left)

        } else {
            return this.toArrPreorder(arr, this.root.left)
        }

    }
    toArrPre(node = this.root, arr = []){
        if(node){
            arr.push(node.data);
            this.toArrPre(node.left, arr);
            this.toArrPre(node.right, arr);
        }
        return arr;
    }
    toArrinOrder()
